...
const Details = () => {
    ...
    const job = jobs.find((job) => job.bookId === int);
    const {bookName, author, image, review, totalPages, rating, category, tags, publisher, yearOfPublishing} = job;
  return (
    <div className="grid grid-cols-12 gap-[48px]">
      ...
      <div className="col-span-6">
        <div>
          ...
          <h2 className="ws font-medium text-[20px] text-[#424242] mb-[24px]">By : {author}</h2>
        </div>
        <hr />
        <div>
        ...
        <div className="py-[24px]">
          <p className="ws font-bold text-[16px]">Review : <span className="font-normal text-[16px] text-[#5A5A5A]">{review}</span>
          </p>
        </div>
        <div className="flex items-center gap-5 font-bold mb-[24px]">
          ...
          {
                        tags.map((tag,index) => <h2 className="ws font-medium px-[16px] py-[7px] bg-[#F4FCF3] text-[#23BE0A] rounded-[30px]"><span># </span>
                        {tag}
                    </h2>)
                    }
          </div>
        ...
        <div className="flex gap-5 mt-[32px]">
            ...
            <button className="btn bg-[#59C6D2] text-white ws hover:bg-[#59C6D2]">Wishlist</button>
        </div>
      </div>
    </div>
  );
